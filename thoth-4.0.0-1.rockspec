package = "thoth"
version = "4.0.0-1"
source = {
   url = "git+https://github.com/gongahkia/thoth.git",
   tag = "4.0.0"
}
description = {
   summary = "Functional Lua pocket knife with cross-framework game runtime adapters",
   homepage = "https://github.com/gongahkia/thoth",
   license = "MIT"
}
dependencies = {
   "lua >= 5.1"
}
build = {
   type = "builtin",
   modules = {
      ["thoth"] = "thoth.lua",
      ["init"] = "init.lua",
      ["thoth.addons"] = "thoth/addons/init.lua",
      ["thoth.addons.gameplay"] = "thoth/addons/gameplay/init.lua",
      ["thoth.addons.gameplay.commands"] = "thoth/addons/gameplay/commands.lua",
      ["thoth.addons.gameplay.cooldowns"] = "thoth/addons/gameplay/cooldowns.lua",
      ["thoth.addons.terrain"] = "thoth/addons/terrain.lua",
      ["thoth.addons.gameplay.resources"] = "thoth/addons/gameplay/resources.lua",
      ["thoth.addons.gameplay.status"] = "thoth/addons/gameplay/status.lua",
      ["thoth.core"] = "thoth/core/init.lua",
      ["thoth.core.api"] = "thoth/core/api.lua",
      ["thoth.core.cache"] = "thoth/core/cache.lua",
      ["thoth.core.config"] = "thoth/core/config.lua",
      ["thoth.core.datetime"] = "thoth/core/datetime.lua",
      ["thoth.core.deques"] = "thoth/core/deques.lua",
      ["thoth.core.events"] = "thoth/core/events.lua",
      ["thoth.core.graphs"] = "thoth/core/graphs.lua",
      ["thoth.core.heaps"] = "thoth/core/heaps.lua",
      ["thoth.core.links"] = "thoth/core/links.lua",
      ["thoth.core.orderedmaps"] = "thoth/core/orderedmaps.lua",
      ["thoth.core.math"] = "thoth/core/math.lua",
      ["thoth.core.math2D"] = "thoth/core/math2D.lua",
      ["thoth.core.math2d"] = "thoth/core/math2d.lua",
      ["thoth.core.logging"] = "thoth/core/logging.lua",
      ["thoth.core.performance"] = "thoth/core/performance.lua",
      ["thoth.core.path"] = "thoth/core/path.lua",
      ["thoth.core.queues"] = "thoth/core/queues.lua",
      ["thoth.core.ringbuffers"] = "thoth/core/ringbuffers.lua",
      ["thoth.core.serialize"] = "thoth/core/serialize.lua",
      ["thoth.core.sets"] = "thoth/core/sets.lua",
      ["thoth.core.stacks"] = "thoth/core/stacks.lua",
      ["thoth.core.stringify"] = "thoth/core/stringify.lua",
      ["thoth.core.tables"] = "thoth/core/tables.lua",
      ["thoth.core.trees"] = "thoth/core/trees.lua",
      ["thoth.core.tries"] = "thoth/core/tries.lua",
      ["thoth.core.unionfind"] = "thoth/core/unionfind.lua",
      ["thoth.core.validate"] = "thoth/core/validate.lua",
      ["thoth.game"] = "thoth/game/init.lua",
      ["thoth.game.animation"] = "thoth/game/animation.lua",
      ["thoth.game.behavior"] = "thoth/game/behavior.lua",
      ["thoth.game.camera"] = "thoth/game/camera.lua",
      ["thoth.game.ecs"] = "thoth/game/ecs.lua",
      ["thoth.game.collision"] = "thoth/game/collision.lua",
      ["thoth.game.frame"] = "thoth/game/frame.lua",
      ["thoth.game.input"] = "thoth/game/input.lua",
      ["thoth.game.navigation"] = "thoth/game/navigation.lua",
      ["thoth.game.pathfinding"] = "thoth/game/pathfinding.lua",
      ["thoth.game.terrain"] = "thoth/game/terrain/init.lua",
      ["thoth.game.terrain.evolution"] = "thoth/game/terrain/evolution.lua",
      ["thoth.game.terrain.export"] = "thoth/game/terrain/export.lua",
      ["thoth.game.terrain.grid"] = "thoth/game/terrain/grid.lua",
      ["thoth.game.terrain.noise"] = "thoth/game/terrain/noise.lua",
      ["thoth.game.terrain.palette"] = "thoth/game/terrain/palette.lua",
      ["thoth.game.terrain.registry"] = "thoth/game/terrain/registry.lua",
      ["thoth.game.terrain.simulation"] = "thoth/game/terrain/simulation.lua",
      ["thoth.game.tilemap"] = "thoth/game/tilemap.lua",
      ["thoth.game.random"] = "thoth/game/random.lua",
      ["thoth.game.runtime"] = "thoth/game/runtime.lua",
      ["thoth.game.spatial"] = "thoth/game/spatial.lua",
      ["thoth.game.state"] = "thoth/game/state.lua",
      ["thoth.game.tasks"] = "thoth/game/tasks.lua",
      ["thoth.game.tween"] = "thoth/game/tween.lua",
      ["thoth.adapters"] = "thoth/adapters/init.lua",
      ["thoth.adapters.contract"] = "thoth/adapters/contract.lua",
      ["thoth.adapters.love2d"] = "thoth/adapters/love2d.lua",
      ["thoth.adapters.defold"] = "thoth/adapters/defold.lua",
      ["thoth.adapters.solar2d"] = "thoth/adapters/solar2d.lua"
   }
}
