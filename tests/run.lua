...
tests[#tests + 1] = function()
    ...
    expect(Render.cutsceneForEvent({ message = "combat: regent", event = "boss_start", boss = true, enemies = { "Vault Regent" } }, sim).kind == "boss_intro", "boss start should map to boss intro")
    expect(Render.cutsceneForEvent({ message = "Vault Regent used Sentence", event = "boss_skill", actor = "Vault Regent", skill = "Sentence", boss = true }, sim).kind == "boss_strike", "boss skill should map to boss strike")
